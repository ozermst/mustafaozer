...
class Header(ClusterableModel, TranslatableMixin):
    ...
    panels = [
        ...
        InlinePanel("header_menu_items", label=_("Menu items")),
    ]
...
class HeaderMenuItem(TranslatableMixin, Orderable):

    parent = ParentalKey("Header", on_delete=models.CASCADE, related_name="header_menu_items")
    ...
    class Meta:
        ordering = ["sort_order"]
        verbose_name = _("Header menu item")
        verbose_name_plural = _("Header menu items")
        unique_together = ("translation_key", "locale")
